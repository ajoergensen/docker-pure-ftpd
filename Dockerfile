...
RUN \
	apk -U update && \
	apk add --virtual .builddeps libressl-dev build-base libcap-dev && \
	cd /tmp && \
	...
	./configure --prefix=/usr --without-humor --without-inetd --with-throttling --with-puredb --with-tls --with-altlog && \
